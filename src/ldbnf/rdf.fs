...
module RdfUris =
  ...
  type Uri with
    ...
    static member from (TheraputicIndication s) = !!(Uri.nicebnfClass + "Indication#" + (NameUtils.niceCamelName s))
    static member fromc (Classification (Id s,_)) = !!(Uri.nicebnfClass + "Classification#" + s)
    static member froms s = !!(Uri.nicebnfClass + "Specificity#" + (NameUtils.niceCamelName s))
